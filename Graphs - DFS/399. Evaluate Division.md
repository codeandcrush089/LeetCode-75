...
![Medium](https://img.shields.io/badge/Level-Medium-yellow?style=for-the-badge) [![Problem Link](https://img.shields.io/badge/LeetCode-Link-orange?style=for-the-badge&logo=leetcode)](https://leetcode.com/problems/evaluate-division/description/)
...
* Model each equation as a graph edge:

  * `A → B` with weight `A / B`
  * `B → A` with weight `1 / (A / B)`
* For each query, perform DFS to find a path from the numerator to the denominator.
* Multiply edge weights along the path to compute the result.
* Use a visited set to avoid cycles.
* If no valid path exists, return `-1.0`.
