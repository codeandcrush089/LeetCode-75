...
* Build combinations by choosing numbers from `1` to `9` without repetition.
* At each step, reduce:

  * `k` → remaining numbers to pick
  * `n` → remaining target sum
* Stop early if `k` becomes `0` or `n` goes negative.
* When both `k` and `n` reach `0`, record the valid combination.
