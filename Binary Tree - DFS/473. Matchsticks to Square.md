...
* **Depth-First Search (DFS) with Prefix Sum + Hash Map**
* Traverse the tree using DFS while maintaining a running sum from the root.
* Use a hash map to store how many times each prefix sum has occurred.
* For each node, check how many previous prefix sums satisfy
  `currentSum − targetSum`.
* Add the current sum before exploring children and **backtrack** after returning.
* Accumulate counts of all valid paths.
