...
* **Greedy + Min-Heap**
* Pair values from both arrays and sort them by `nums2` in descending order.
* Treat the current `nums2` value as the minimum factor.
* Use a min-heap to keep the top `k` largest `nums1` values.
* Maintain the running sum of selected `nums1`.
* At each step where `k` elements are selected, compute the score and update the maximum.
