...
* **Two Min-Heaps with Two Pointers (Greedy Selection)**
* Use two pointers from the left and right ends of the array.
* Maintain two min-heaps:

  * Left heap for the first `candidates` workers.
  * Right heap for the last `candidates` workers.
* For each of the `k` selections:

  * Refill heaps using pointers when needed.
  * Choose the smaller cost from the tops of the two heaps.
* Add the chosen cost to the total.
